        ...
        while ($val = $db->fetchByAssoc($result, false)) {
            if ($members) {
                ...
                if ($focus->table_name == 'leads'){
                    ...
                    //$query_name_campaign = "SELECT name FROM campaigns WHERE id = '{$campaign_id}'";
                    //$result_query_campaign = $GLOBALS['db']->query($query_name_campaign);
                    //$name_campaign = $GLOBALS['db']->fetchByAssoc($result_query_campaign);
                    //$val['campaign_id'] = $name_campaign['name'];

                    $assigned_user_id = $val['assigned_user_id'];
                    $query_name_user = "SELECT user_name FROM users WHERE id = '{$assigned_user_id}'";
                    $result_query_user= $GLOBALS['db']->query($query_name_user);
                    $name_user = $GLOBALS['db']->fetchByAssoc($result_query_user);
                    $val['assigned_user_id'] = $name_user['user_name'];
...
function translateForExport($field_db_name, $focus)
{
    ...
    //if ($focus->table_name == 'leads'){
    //    if (strtoupper($field_db_name) == 'CAMPAIGN_ID'){
    //        return $mod_strings['LBL_CAMPAIGNS'];
    //    }
    //}
